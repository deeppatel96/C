...
void updateBits(Cache *cache, char *addr){
    memcpy(cache->currTag, addr, cache->tag_bits);
    memcpy(cache->currIndex, &addr[cache->tag_bits], cache->index_bits);
...
void printCache(Cache *cache){
    ...
    for(i = 0; i < cache->num_lines; i++){
        printf("Line:%3d   |Set:%3d  |Rank:%2d  |Valid:%d |Dirty:%d  |Tag: %s\n", i, i / cache->set_size , cache->rank[i], cache->valid[i],cache->dirty[i], cache->tag[i]);
    }
